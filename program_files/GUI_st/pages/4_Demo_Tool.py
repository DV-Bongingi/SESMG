...
def dt_input_sidebar():
    ...
    with st.sidebar.form("Simulation input"):
        ...
        input_values_dict["input_battery"] = st.number_input(
            label="Battery in kWh", min_value=0, max_value=10000, step=1
        )
        ...
        input_values_dict["input_gchp"] = st.number_input(
            label="Heat Pump in kW", min_value=0, max_value=5000, step=1
        )
        ...
        input_dh = st.checkbox(label="District Heating Network")
...
def demo_start_page():
    ...
    imported_markdown = read_markdown_document(
        document_path="docs/GUI_texts/demo_tool.md", folder_path=""
    )
